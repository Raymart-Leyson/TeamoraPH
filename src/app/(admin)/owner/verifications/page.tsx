...
export default async function AdminVerificationsPage() {
    ...
    return (
        <div className="space-y-10">
            <div>
                <h1 className="text-2xl sm:text-4xl font-black text-[#1B3FA0] tracking-tight">User Verifications</h1>
                <p className="text-[#1B3FA0]/70 font-bold mt-2">Verify identity documents and manage verification badges.</p>
            </div>
            ...
            <div className="grid gap-6">
                {requests && requests.length > 0 ? (
                            ...
                            <CardContent className="p-8">
                                <div className="flex flex-col md:flex-row gap-8">
                                    ...
                                    <div className="flex flex-row md:flex-col items-center md:items-start gap-4 md:w-48 shrink-0">
                                        <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-[#1B3FA0]/5 flex items-center justify-center border border-[#1B3FA0]/10">
                                            <User className="w-8 h-8 text-[#1B3FA0]/40" />
                                        </div>
                                        <div className="min-w-0 space-y-2">
                                            <p className="font-black text-[#1B3FA0] truncate capitalize">{request.user.role}</p>
                                            <p className="text-xs font-bold text-[#1B3FA0]/50 truncate">{request.user.email}</p>
                                            {/* View Profile Link */}
                                            {request.user.role === 'candidate' ? (
                                                <Button asChild variant="outline" size="sm" className="rounded-xl font-bold text-xs h-8 border-[#1B3FA0]/20 hover:bg-[#1B3FA0] hover:text-white">
                                    ...
                                    <div className="flex-1 space-y-6">
                                        <div className="flex justify-between items-start gap-4">
                                            <div>
                                                <h3 className="text-2xl font-black text-[#1B3FA0] leading-tight capitalize">
                                                    {request.type.replace('_', ' ')} Verification
                                                </h3>
                                                <div className="flex items-center gap-1.5 mt-2 text-xs font-bold text-[#1B3FA0]/60">
                                        ...
                                        <div className="space-y-3">
                                            <p className="text-xs font-black uppercase tracking-widest text-[#1B3FA0]/40">
                                            ...
                                            {request.documents && request.documents.length > 0 ? (
                                                request.type === 'social_link' ? (
                                                    <a
                                                        ...
                                                        target="_blank"
                                                        rel="noreferrer"
                                                        className="flex items-center gap-2 px-4 py-2 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow text-sm font-bold text-[#1B3FA0] w-fit"
                                                    >
                                                        ...
                                                        {request.documents.map((doc: string, idx: number) => {
                                                            ...
                                                            return (
                                                                <a key={idx} href={doc} target="_blank" rel="noreferrer" className="group flex flex-col gap-1">
                                                                    <div className="w-40 h-28 rounded-xl overflow-hidden border border-slate-200 bg-slate-100 group-hover:border-[#1B3FA0]/40 transition-colors">
                                                                        <img src={doc} alt={label} className="w-full h-full object-cover" />
                                                                    </div>
                                                                    <p className="text-[10px] font-bold text-center text-[#1B3FA0]/50">{label}</p>
                                                                </a>
                                                            );
                                                        })}
                                                    </div>
                                                ...
                                                <p className="text-sm font-bold text-[#1B3FA0]/40 italic">No documents provided.</p>
                                            )}
                                        </div>
                    ...
                    <div className="bg-white/30 backdrop-blur-sm border-2 border-dashed border-white/40 rounded-[3rem] p-20 flex flex-col items-center justify-center text-center">
                        ...
                        <h2 className="text-2xl font-black text-[#1B3FA0]">Perfect Score!</h2>
                        <p className="text-[#1B3FA0]/50 font-bold mt-2">Zero pending verification requests.</p>
                    </div>
                )}
            </div>
        </div>
    ...
    return (
        <Dialog>
            ...
            <DialogContent className="rounded-[2rem] border-none shadow-2xl bg-[#F8F9FF]">
                <DialogHeader>
                    <DialogTitle className="text-2xl font-black text-[#1B3FA0]">Reject Verification</DialogTitle>
