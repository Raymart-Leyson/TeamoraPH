    ...
    return (
        <div className="space-y-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-2xl sm:text-4xl font-black text-[#1B3FA0] tracking-tight">User Management</h1>
                    <p className="text-[#1B3FA0]/70 font-bold mt-2">Manage user accounts, roles, and platform permissions.</p>
                </div>
            ...
            <div className="grid gap-4">
                {users && users.length > 0 ? (
                                            ...
                                            <tr key={user.id} className="hover:bg-white/40 transition-colors group">
                                                <td className="px-8 py-5">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center font-black text-[#1B3FA0] text-xs">
                                                            {user.email[0].toUpperCase()}
                                                        </div>
                                                        <div>
                                                            <div className="font-black text-[#1B3FA0]">{user.email}</div>
                                                            <div className="text-[10px] font-bold text-[#1B3FA0]/40 uppercase tracking-tighter">{user.id.split('-')[0]}...</div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-8 py-5">
                                                    ...
                                                    <DropdownMenu>
                                                        <DropdownMenuTrigger asChild>
                                                            <Button variant="ghost" size="icon" className="rounded-xl hover:bg-white/60">
                                                                <MoreVertical className="w-5 h-5 text-[#1B3FA0]/40" />
                                                            </Button>
                                                        </DropdownMenuTrigger>
                                                        <DropdownMenuContent align="end" className="w-56 rounded-2xl border-none shadow-2xl bg-[#F8F9FF] p-2">
                                                            <DropdownMenuLabel className="px-3 py-2 text-xs font-black uppercase tracking-widest text-[#1B3FA0]/40">Update Role</DropdownMenuLabel>
                                                            <DropdownMenuSeparator className="bg-white/40 my-1" />
                                                            {['candidate', 'employer', 'staff', 'admin'].map((role) => (
                                                                <DropdownMenuItem
                                                                    key={role}
                                                                    disabled={user.role === role}
                                                                    className="rounded-xl font-bold text-[#1B3FA0] focus:bg-white focus:text-[#1B3FA0] cursor-pointer"
                                                                >
                                                                    <form action={async () => {
                                                                        "use server";
                    ...
                    <div className="bg-white/30 backdrop-blur-sm border-2 border-dashed border-white/40 rounded-[3rem] p-20 flex flex-col items-center justify-center text-center">
                        <Users className="w-12 h-12 text-[#1B3FA0]/20 mb-4" />
                        <h2 className="text-2xl font-black text-[#1B3FA0]">No users found</h2>
                        <p className="text-[#1B3FA0]/50 font-bold mt-2">Try searching for a different email address.</p>
                    </div>
                )}
            </div>
        </div>
