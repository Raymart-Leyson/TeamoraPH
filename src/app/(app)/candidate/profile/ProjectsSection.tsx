                        ...
                        <CardContent className="p-6 space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <Label htmlFor="title" className="text-[#1B3FA0] font-semibold">Project Title *</Label>
                                    <Input id="title" name="title" required className="shadow-inner" placeholder="e.g. Ecommerce Platform" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="role_in_project" className="text-[#1B3FA0] font-semibold">Your Role *</Label>
                                    <Input id="role_in_project" name="role_in_project" required className="shadow-inner" placeholder="e.g. Lead Designer" />
                                </div>
                                <div className="space-y-2 md:col-span-2">
                                    <Label htmlFor="url" className="text-[#1B3FA0] font-semibold">Live URL / Link</Label>
                                    <Input id="url" name="url" type="url" className="shadow-inner" placeholder="https://..." />
                                </div>
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="description" className="text-[#1B3FA0] font-semibold">Description *</Label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {items.length === 0 && !isAdding && (
                    <div className="col-span-full p-8 bg-white/40 border border-[#1B3FA0]/10 rounded-[2rem] text-center">
                        <p className="text-[#1B3FA0]/60 font-medium">No projects added yet.</p>
                    </div>
                )}
                {items.map((item) => (
                    <div key={item.id} className="p-6 rounded-[1.5rem] bg-white border border-[#1B3FA0]/10 shadow-sm relative group flex flex-col justify-between">
                        <div>
                            <div className="flex items-start justify-between">
                                <h4 className="font-bold text-lg text-[#1B3FA0] leading-tight">
                                    {item.title}
                                </h4>
                            </div>
                            <p className="text-sm font-bold text-[#3D6EFF] mt-1">{item.role_in_project}</p>
                            <p className="text-sm text-slate-500 mt-3 whitespace-pre-wrap">{item.description}</p>
                        </div>
                        {item.url && (
                            <div className="mt-4 pt-4 border-t border-[#1B3FA0]/10">
                                <a href={item.url} target="_blank" rel="noreferrer" className="text-[#1B3FA0] text-sm font-bold hover:underline flex items-center gap-1 w-max">
