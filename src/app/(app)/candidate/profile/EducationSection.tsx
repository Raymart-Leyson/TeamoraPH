            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {items.length === 0 && !isAdding && (
                    <div className="col-span-full p-8 bg-white/40 border border-[#1B3FA0]/10 rounded-[2rem] text-center">
                        <p className="text-[#1B3FA0]/60 font-medium">No education added yet.</p>
                    </div>
                )}
                {items.map((item) => (
                    <div key={item.id} className="p-5 rounded-[1.5rem] bg-white border border-[#1B3FA0]/10 shadow-sm relative group">
                        <div className="flex justify-between items-start">
                            <div>
                                <h4 className="font-bold text-lg text-[#1B3FA0] leading-tight">
                                    {item.degree_level} in {item.field_of_study}
                                </h4>
                                <p className="text-[#3D6EFF] font-semibold flex items-center gap-1 mt-1">
