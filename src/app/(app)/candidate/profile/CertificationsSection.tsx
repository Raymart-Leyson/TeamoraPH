                        ...
                        <CardContent className="p-6 space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <Label htmlFor="name" className="text-[#1B3FA0] font-semibold">Certificate Name *</Label>
                                    <Input id="name" name="name" required className="shadow-inner" placeholder="e.g. AWS Certified..." />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="issuing_org" className="text-[#1B3FA0] font-semibold">Issuing Organization *</Label>
                                    <Input id="issuing_org" name="issuing_org" required className="shadow-inner" placeholder="e.g. Amazon Web Services" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="issue_date" className="text-[#1B3FA0] font-semibold">Issue Date</Label>
                                    <Input type="date" id="issue_date" name="issue_date" className="shadow-inner" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="credential_url" className="text-[#1B3FA0] font-semibold">Credential URL</Label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {items.length === 0 && !isAdding && (
                    <div className="col-span-full p-8 bg-white/40 border border-[#1B3FA0]/10 rounded-[2rem] text-center">
                        <p className="text-[#1B3FA0]/60 font-medium">No certifications added yet.</p>
                    </div>
                )}
                {items.map((item) => (
                    <div key={item.id} className="p-5 rounded-[1.5rem] bg-white border border-[#1B3FA0]/10 shadow-sm relative group flex flex-col justify-between">
                        <div>
                            <h4 className="font-bold text-[#1B3FA0] leading-tight">
                                {item.name}
                            </h4>
                        ...
                        {item.credential_url && (
                            <a href={item.credential_url} target="_blank" rel="noreferrer" className="text-[#3D6EFF] text-xs font-bold hover:underline bg-[#3D6EFF]/10 px-3 py-1.5 rounded-full inline-flex items-center gap-1 max-w-max">
