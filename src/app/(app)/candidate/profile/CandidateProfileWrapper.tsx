        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col lg:flex-row items-stretch gap-8 mb-4">
                ...
                <Card className="flex-1 bg-white/50 backdrop-blur-xl border-white/60 shadow-xl rounded-[2.5rem] overflow-hidden relative">
                    <div className="h-32 w-full bg-gradient-to-r from-[#1B3FA0] to-[#1B3FA0]/80" />
                    <CardContent className="px-8 pb-8 pt-0 flex flex-col items-start">
                        <div className="flex justify-between w-full mt-[-3rem]">
                            <Avatar className="h-28 w-28 border-4 border-[#F8F9FF] shadow-lg bg-white">
                            ...
                            {defaults.availability && (
                                <div className="mt-14 shrink-0">
                                    <Badge variant="outline" className="bg-[#1B3FA0]/5 text-[#1B3FA0] border-[#1B3FA0]/20 font-bold py-1.5 px-3">
                                        {defaults.availability}
                                    </Badge>
                                </div>
                            )}
                        ...
                        <div className="mt-5 w-full">
                            <h1 className="text-3xl md:text-4xl font-extrabold text-[#1B3FA0] tracking-tight">{fullName}</h1>
                            {defaults.headline && (
                                <h2 className="text-xl font-bold text-[#3D6EFF] mt-2">{defaults.headline}</h2>
                            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    {experience && experience.length > 0 && (
                        <Card className="bg-white/50 backdrop-blur-xl border-white/60 shadow-xl rounded-[2rem]">
                            <CardContent className="p-8">
                                <h3 className="text-2xl font-bold text-[#1B3FA0] tracking-tight mb-8 border-b border-slate-200 pb-4 flex items-center">
                                    <BriefcaseBusiness className="w-6 h-6 mr-3 text-[#3D6EFF]" /> Experience
                                </h3>
                                <div className="space-y-8 border-l-2 border-slate-200 ml-4 pb-4">
                                    {experience.map((exp: any) => (
                                        <div key={exp.id} className="relative pl-8">
                                            <div className="absolute w-4 h-4 bg-[#3D6EFF] rounded-full -left-[9px] top-1.5 ring-4 ring-[#F8F9FF]"></div>
                                            <h4 className="text-xl font-bold text-[#1B3FA0] leading-tight">{exp.job_title}</h4>
                                            <p className="font-bold text-[#3D6EFF] mt-1 text-sm">{exp.company_name}</p>
                    ...
                    {education && education.length > 0 && (
                        <Card className="bg-white/50 backdrop-blur-xl border-white/60 shadow-xl rounded-[2rem]">
                            <CardContent className="p-8">
                                <h3 className="text-2xl font-bold text-[#1B3FA0] tracking-tight mb-8 border-b border-slate-200 pb-4 flex items-center">
                                    <GraduationCap className="w-6 h-6 mr-3 text-[#3D6EFF]" /> Education
                                </h3>
                                <div className="space-y-6">
                                    {education.map((edu: any) => (
                                        <div key={edu.id} className="bg-white/60 p-5 rounded-2xl border border-white shadow-sm flex items-start gap-4">
                                            <div className="w-12 h-12 rounded-xl bg-[#1B3FA0]/5 text-[#1B3FA0] flex items-center justify-center shrink-0">
                                                <GraduationCap className="w-6 h-6" />
                                            </div>
                                            <div>
                                                <h4 className="text-lg font-bold text-[#1B3FA0] leading-tight">{edu.school_name}</h4>
                    ...
                    {projects && projects.length > 0 && (
                        <Card className="bg-white/50 backdrop-blur-xl border-white/60 shadow-xl rounded-[2rem]">
                            <CardContent className="p-8">
                                <h3 className="text-2xl font-bold text-[#1B3FA0] tracking-tight mb-8 border-b border-slate-200 pb-4 flex items-center">
                                    <Code className="w-6 h-6 mr-3 text-[#3D6EFF]" /> Highlighted Projects
                                </h3>
                                    ...
                                    {projects.map((proj: any) => (
                                        <div key={proj.id} className="bg-white/60 p-5 rounded-2xl border border-white shadow-sm flex flex-col group hover:shadow-md transition-shadow">
                                            <div className="flex justify-between items-start mb-2">
                                                <h4 className="text-lg font-bold text-[#1B3FA0] group-hover:text-[#3D6EFF] transition-colors">{proj.title}</h4>
                                                {proj.url && (
                                                    <a href={proj.url} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-[#1B3FA0] transition-colors">
                    ...
                    {experience.length === 0 && education.length === 0 && projects.length === 0 && (
                        <Card className="bg-white/50 backdrop-blur-xl border-white/60 shadow-xl rounded-[2rem]">
                            <CardContent className="p-12 text-center text-[#1B3FA0]/40">
                ...
                <div className="space-y-8">
                    {skills && skills.length > 0 && (
                        <Card className="bg-white/50 backdrop-blur-xl border-white/60 shadow-xl rounded-[2rem]">
                            <CardContent className="p-8">
                                <h3 className="text-xl font-bold text-[#1B3FA0] tracking-tight mb-6 border-b border-slate-200 pb-4">
                                    Skills & Expertise
                                </h3>
                                <div className="space-y-4">
                                                ...
                                                <div className="flex items-center gap-0.5">
                                                    {[1, 2, 3, 4, 5].map((star) => (
                                                        <Star
                                                            ...
                                                            className={cn(
                                                                "h-3.5 w-3.5",
                                                                skill.rating >= star ? "fill-[#3D6EFF] text-[#3D6EFF]" : "text-slate-200"
                                                            )}
                                                        />
                                                    ))}
                                                </div>
                                ...
                                <div className="space-y-4">
                                    {certifications.map((cert: any) => (
                                        <div key={cert.id} className="flex items-start gap-3 bg-white/60 p-4 rounded-xl shadow-sm border border-white">
                                            <Award className="w-5 h-5 text-[#3D6EFF] shrink-0 mt-0.5" />
                                            <div>
                                                <h4 className="font-bold text-[#1B3FA0] text-sm leading-tight">{cert.name}</h4>
                    ...
                    {defaults.resume_url && (
                        <Card className="bg-gradient-to-br from-[#1B3FA0] to-[#1B3FA0]/80 text-white shadow-xl rounded-[2rem] border-none overflow-hidden relative">
                            ...
                            <CardContent className="p-8 relative z-10">
                                ...
                                <Button className="w-full bg-white text-[#1B3FA0] hover:bg-slate-100 font-bold rounded-xl" asChild>
        ...
        <div className="space-y-6">
            ...
            {mode === "edit" && (
                <div className="space-y-6 pb-6">
                    ...
                    <div className="h-px w-full bg-[#1B3FA0]/10" />
